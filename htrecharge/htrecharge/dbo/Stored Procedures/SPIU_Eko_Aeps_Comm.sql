...
-- Create date: 01-Dec-2021
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[SPIU_Eko_Aeps_Comm]
	@memberid bigint,
	@txnid bigint,
	@txndate datetime,
	@IP varchar(50) = null
...
BEGIN
	...
	Select @txnamt = amount From T_Eko_Aeps_Txn Where mem_id = @memberid And autoid = @txnid And tx_status = 0
	...
	*/
	Set @Service_Category_Id = 24 ---Is Only for Aeps From Table "M_Service_Type_Sub_Category" Column "Service_Sub_Category_Id"
	...
	Begin
		...
		Set @TxnComment = 'Aeps Commission @'+Convert(Varchar(10), @CommPer)+'% On Amount '+Convert(Varchar(10), @CommOnAmt)+' by Member Code ' + @TempMemberCode

		--Credit Txn Amt
		set @OpeningBal = IsNull((select Payout_Wallet From T_Member where Mem_ID = @TempMemId), 0)
		set @ClosingBal = (@OpeningBal + @CommAmt)

		if not exists (select pwt_id from T_PayoutWallet where pwt_memid = @TempMemId and pwt_servicerefid = @txnid and pwt_datetime = @txndate And pwt_comment Like 'Aeps Commission%')
		begin
