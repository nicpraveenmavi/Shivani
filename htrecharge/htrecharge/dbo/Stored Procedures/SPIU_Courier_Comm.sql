...
-- Create date: 18-Dec-2021
-- Alter date: 22-Dec-2021
-- Description:	<Description,,>
-- =============================================
CREATE PROCEDURE [dbo].[SPIU_Courier_Comm]
	@memberid bigint,
	@txnid bigint,
	@txndate datetime,
	@IP varchar(50) = null
...
BEGIN
	...
	Select @txnamt = Member_Courier_Charge From T_CourierOrder Where MemId = @memberid And Order_Id = @txnid And CourierStatus = 1
	...
	*/
	Set @Service_Category_Id = 26 ---Is Only for Aeps From Table "M_Service_Type_Sub_Category" Column "Service_Sub_Category_Id"
	...
	Begin
		...
		Set @TxnComment = 'Courier Booking Commission @'+Convert(Varchar(10), @CommPer)+'% On Amount '+Convert(Varchar(10), @CommOnAmt)+' by Member Code ' + @TempMemberCode

		--Credit Txn Amt
		set @OpeningBal = IsNull((select Payout_Wallet From T_Member where Mem_ID = @TempMemId), 0)
		set @ClosingBal = (@OpeningBal + @CommAmt)

		if not exists (select pwt_id from T_PayoutWallet where pwt_memid = @TempMemId and pwt_servicerefid = @txnid and pwt_datetime = @txndate And pwt_comment Like 'Courier Booking Commission%' And pwt_Credit > 0)
		begin
